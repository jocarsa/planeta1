...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier):
    ...
    section = Image.new("RGBA", (x_end - x_start, y_end - y_start))
    ...
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            ...
            if 0 <= iso_x < iso_width and 0 <= iso_y < iso_height:
                section.putpixel((iso_x, iso_y), color + (255,))
                depth_section.putpixel((iso_x, iso_y), int(normalized_value * 65535))  # Depth as 16-bit grayscale intensity

    # Generate cloud layer using Perlin noise
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            # Cloud Perlin noise value
            cloud_value = noise.pnoise2((x + seed) / 100.0, (y + seed) / 100.0, octaves=8, persistence=0.5, lacunarity=2.0)
            cloud_alpha = int((cloud_value + 1) / 2 * 255)  # Normalize to [0, 255]
            
            # Add clouds with some transparency (cloud_alpha determines cloud density)
            if cloud_alpha > 128:  # Only draw clouds where density is significant
                cloud_color = (255, 255, 255, cloud_alpha)
                iso_x = int((x - x_start - (y - y_start)) * math.sqrt(3) / 2) + center_x
                iso_y = int((x - x_start + (y - y_start)) / 2 - height_multiplier - 100) + center_y - section_size // 2

                if 0 <= iso_x < iso_width and 0 <= iso_y < iso_height:
                    existing_color = section.getpixel((iso_x, iso_y))
                    new_color = tuple(
                        int((cloud_alpha / 255) * c + (1 - cloud_alpha / 255) * e) 
                        for c, e in zip(cloud_color[:3], existing_color[:3])
                    )
                    section.putpixel((iso_x, iso_y), new_color + (255,))
