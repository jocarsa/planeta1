...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur, y_pixel_offset, pixel_scale):
    random.seed(seed)
    
    iso_width, iso_height = (x_end - x_start) * pixel_scale, (y_end - y_start) * pixel_scale
    ...
    for row in data:
        ...
        iso_x = int(((x - x_start - (y - y_start)) * math.sqrt(3) / 2) * pixel_scale) + center_x
        iso_y = int(((x - x_start + (y - y_start)) / 2 - normalized_value * height_multiplier) * pixel_scale) + center_y - int(height_multiplier * pixel_scale * 0.65) + y_pixel_offset
...
section_size = int(math.sqrt(0.0001 * width * height)) * 2  # Increase the section size by a factor of 2
...
def update_canvas():
    global x_start, y_start, section_size, canvas, tk_img, cur, height_multiplier, y_pixel_offset, pixel_scale
    x_end = x_start + section_size
    y_end = y_start + section_size
    
    # Generate the section in isometric perspective
    section = generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur, y_pixel_offset, pixel_scale)
...
# Pixel scale factor for isometric projection
pixel_scale = 2
...
canvas = Canvas(root, width=section_size * pixel_scale, height=section_size * pixel_scale)
canvas.pack()
...
slider_y_pixel_offset = Scale(root, from_=-section_size//2, to=section_size//2, orient=HORIZONTAL, label="Y Pixel Offset", command=update_y_pixel_offset)
