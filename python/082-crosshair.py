...
def draw_sphere():
    ...
    def initial_rotation(lat, lon):
        # Rotate the sphere by 90 degrees around the x-axis
        x = math.cos(lat) * math.cos(lon)
        y = math.cos(lat) * math.sin(lon)
        z = math.sin(lat)
    ...
    for i in range(num_parallels):
        ...
        for j in range(num_meridians):
            ...
            lat1_rot, lon1_rot = rotate_local(lat1, lon1, -y_start * 2 * math.pi / height, -x_start * 2 * math.pi / width)
            lat2_rot, lon2_rot = rotate_local(lat2, lon2, -y_start * 2 * math.pi / height, -x_start * 2 * math.pi / width)
            lat3_rot, lon3_rot = rotate_local(lat2, lon1, -y_start * 2 * math.pi / height, -x_start * 2 * math.pi / width)
            lat4_rot, lon4_rot = rotate_local(lat1, lon2, -y_start * 2 * math.pi / height, -x_start * 2 * math.pi / width)
...
def draw_equirectangular_map():
    ...
    # Create a low-resolution equirectangular map
    eq_width, eq_height = 512, 256
    ...
    # Draw the crosshair for the current lat/lon position
    eq_x = int((x_start + section_size // 2) * eq_width / width)
    eq_y = int((y_start + section_size // 2) * eq_height / height)
    draw.line([(eq_x - 5, eq_y), (eq_x + 5, eq_y)], fill="red")
    draw.line([(eq_x, eq_y - 5), (eq_x, eq_y + 5)], fill="red")
