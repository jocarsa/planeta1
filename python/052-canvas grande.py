...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier):
    ...
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            ...
            # Calculate isometric coordinates with larger spacing
            iso_x = int((x - x_start - (y - y_start)) * math.sqrt(3) / 2 * 2) + center_x
            iso_y = int((x - x_start + (y - y_start)) / 2 * 2 - normalized_value * height_multiplier * 2) + center_y - section_size + int(height_multiplier * 1.3) // 2
...
def update_canvas():
    ...
    offset_x = (canvas_width - section_size * 2) // 2
    offset_y = (canvas_height - section_size * 2) // 2
...
root.geometry(f"{section_size * 2 + 200}x{section_size * 2 + 200}")

# Create a canvas widget that will expand to fill the window
canvas = Canvas(root, width=section_size * 2, height=section_size * 2)
