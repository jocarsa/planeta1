...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level):
    random.seed(seed)
    ...
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            ...
            iso_x = int((x - x_start - (y - y_start)) * math.sqrt(3) / 2) + iso_width // 2
            iso_y = int((x - x_start + (y - y_start)) / 2 - normalized_value * 50) + iso_height // 2
...
def update_canvas():
    ...
    section = generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level)
