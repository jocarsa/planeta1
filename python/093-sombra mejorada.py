...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur, y_pixel_offset, pixel_separation, cloud_offset, shadow_factor):
    random.seed(seed)
    ...
    # Create a set for faster cloud position lookup
    cloud_positions = set(cloud_dict.keys())
    ...
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            ...
            if len(neighbor_iso) == 3:
                # Check if there is a cloud block directly above the current point and its neighbors
                if (x, y) in cloud_positions or (x + 1, y) in cloud_positions or (x, y + 1) in cloud_positions or (x + 1, y + 1) in cloud_positions:
                    color = darken_color(color, shadow_factor)
...
def update_canvas():
    global x_start, y_start, section_size, canvas, tk_img, cur, height_multiplier, y_pixel_offset, pixel_separation, cloud_offset, shadow_factor
    x_end = x_start + section_size
    y_end = y_start + section_size
    
    # Generate the section in isometric perspective
    section = generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur, y_pixel_offset, pixel_separation, cloud_offset, shadow_factor)
...
# Initialize height multiplier, y pixel offset, cloud offset, and shadow factor
...
label_cloud_offset = ttk.Label(root, text="Cloud Offset")
label_cloud_offset.grid(row=8, column=1, padx=10, pady=(10, 0))
slider_cloud_offset = ttk.Scale(root, from_=50, to=500, orient=HORIZONTAL, command=update_cloud_offset)
slider_cloud_offset.set(cloud_offset)
slider_cloud_offset.grid(row=9, column=1, padx=10, pady=5)

# Create a slider to adjust the shadow factor
label_shadow_factor = ttk.Label(root, text="Shadow Factor")
label_shadow_factor.grid(row=10, column=1, padx=10, pady=(10, 0))
slider_shadow_factor = ttk.Scale(root, from_=0.0, to=1.0, orient=HORIZONTAL, command=update_shadow_factor)
slider_shadow_factor.set(shadow_factor)
slider_shadow_factor.grid(row=11, column=1, padx=10, pady=5)
