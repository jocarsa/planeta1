...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur, y_pixel_offset):
    random.seed(seed)
    ...
    for row in data:
        ...
        iso_y = int((x - x_start + (y - y_start)) / 2 - normalized_value * height_multiplier) + center_y - section_size + int(height_multiplier * 1.3) // 2 + y_pixel_offset
...
section_size = int(math.sqrt(0.0001 * width * height))
...
def update_canvas():
    global x_start, y_start, section_size, canvas, tk_img, cur, height_multiplier, y_pixel_offset
    x_end = x_start + section_size
    y_end = y_start + section_size
    
    # Generate the section in isometric perspective
    section = generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur, y_pixel_offset)
