...
def update_ambient_light():
    ...
    if time_of_day < 12:  # Morning
        shadow_offset_x = int(interpolate_value(-20, 0, time_of_day / 12))
    else:  # Afternoon
        shadow_offset_x = int(interpolate_value(0, 20, (time_of_day - 12) / 12))
...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur, y_pixel_offset, pixel_separation, cloud_offset, shadow_factor, cloud_transparency, cloud_brightness_offset):
    ...
    # Second pass: draw the cloud shadows
    shadow_layer = Image.new("RGBA", (iso_width, iso_height), (0, 0, 0, 0))
    draw_shadows = ImageDraw.Draw(shadow_layer)
    ...
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            ...
            if normalized_value > 0.5:  # Adjust threshold for cloud visibility
                iso_x = int(((x - x_start - (y - y_start)) * math.sqrt(3) / 2) * pixel_separation) + center_x + shadow_offset_x * pixel_separation
                iso_y = int(((x - x_start + (y - y_start)) / 2) * pixel_separation) + center_y + y_pixel_offset
                ...
                for nx, ny in neighbors:
                    if (nx, ny) in cloud_dict:
                        ...
                        n_iso_x = int(((nx - x_start - (ny - y_start)) * math.sqrt(3) / 2) * pixel_separation) + center_x + shadow_offset_x * pixel_separation
                        n_iso_y = int(((nx - x_start + (ny - y_start)) / 2) * pixel_separation) + center_y + y_pixel_offset
                ...
                if len(neighbor_iso) == 3:
                    ...
                    draw_shadows.polygon(shadow_points, fill=(0, 0, 0, 100))  # Dark transparent color for the shadow
    
    # Combine the terrain and shadow layer
    section = Image.alpha_composite(section, shadow_layer)
