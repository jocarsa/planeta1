...
# Initialize global variables for sphere rotation
center_lat = 0.0
center_lon = 0.0
...
# Function to draw a 3D sphere on the left canvas with z-depth sorting
def draw_sphere():
    global cur, sphere_canvas, x_start, y_start, section_size, width, height, center_lat, center_lon
    ...
    for i in range(num_parallels):
        ...
        for j in range(num_meridians):
            ...
            lat1_rot, lon1_rot = rotate(lat1, lon1, center_lat, center_lon)
            lat2_rot, lon2_rot = rotate(lat2, lon2, center_lat, center_lon)
            lat3_rot, lon3_rot = rotate(lat2, lon1, center_lat, center_lon)
            lat4_rot, lon4_rot = rotate(lat1, lon2, center_lat, center_lon)
...
def pan(dx, dy):
    ...
    center_lon = (center_lon + dx * 0.05) % (2 * math.pi)
    center_lat = max(min(center_lat + dy * 0.05, math.pi / 2), -math.pi / 2)
    draw_sphere()
...
btn_up = ttk.Button(btn_frame, text="↑", command=lambda: pan(0, -1))
btn_down = ttk.Button(btn_frame, text="↓", command=lambda: pan(0, 1))
btn_left = ttk.Button(btn_frame, text="←", command=lambda: pan(-1, 0))
btn_right = ttk.Button(btn_frame, text="→", command=lambda: pan(1, 0))
