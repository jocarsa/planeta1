...
def a_star_search(start, goal):
    ...
    while frontier:
        ...
        # Only allow movement in the four cardinal directions
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            next_node = (current[0] + dx, current[1] + dy)
            if 0 <= next_node[0] < width and 0 <= next_node[1] < height:
                cur.execute("SELECT height FROM terrain WHERE x = ? AND y = ?", (next_node[0], next_node[1]))
                result = cur.fetchone()
                if result and result[0] / 65535.0 >= water_level:  # Check if the next node is not water
...
class NPC:
    ...
    def move(self):
        if not self.path:
            ...
            self.path = a_star_search((self.x, self.y), (nearest_food.x, nearest_food.y))
        
        if self.path:
            # Follow the path to the food
