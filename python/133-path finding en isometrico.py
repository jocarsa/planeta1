...
def encontrar_camino(origen, destino):
    global cursor
    open_set = []
    ...
    while open_set:
        ...
        vecinos = [(actual[0] + dx, actual[1] + dy) for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]]
        
        for vecino in vecinos:
            if 0 <= vecino[0] < ancho and 0 <= vecino[1] < alto:
                cursor.execute("SELECT altura FROM terreno WHERE x = ? AND y = ?", vecino)
                resultado = cursor.fetchone()
...
def iniciar_movimiento_personaje(destino_x, destino_y):
    global npcs, x_inicio, y_inicio, tamano_seccion, ancho, alto
    
    # Calcular el camino desde la posición actual del personaje al destino
    camino = encontrar_camino((npcs[0].x, npcs[0].y), (destino_x, destino_y))
    
    if camino:
        # Movemos el personaje paso a paso
        mover_personaje_camino(camino)
    
def mover_personaje_camino(camino):
    if not camino:
        return
    
    siguiente_paso = camino.pop(0)
    
    dx = siguiente_paso[0] - npcs[0].x
    dy = siguiente_paso[1] - npcs[0].y
    
    desplazar(dx, dy, paso=1)
    
    # Continuar moviendo al personaje si quedan pasos en el camino
    if camino:
        raiz.after(1000, lambda: mover_personaje_camino(camino))

# Función para manejar clics en la vista isométrica
def en_clic_terreno(evento):
    global x_inicio, y_inicio, ancho, alto, tamano_seccion, lienzo, cursor
    
    # Obtener la posición del clic en el lienzo
    clic_x = evento.x
    clic_y = evento.y
    
    # Convertir coordenadas de pantalla a coordenadas isométricas
    centro_x, centro_y = tamano_seccion * separacion_pixeles // 2, tamano_seccion * separacion_pixeles // 2
    terreno_x = (clic_x - centro_x + clic_y - centro_y) / (math.sqrt(3) / 2 * separacion_pixeles) + x_inicio
    terreno_y = (clic_y - centro_y - (clic_x - centro_x)) / (math.sqrt(3) / 2 * separacion_pixeles) + y_inicio
    
    # Redondear a la celda más cercana
    terreno_x = int(round(terreno_x))
    terreno_y = int(round(terreno_y))
    
    # Asegurarse de que las coordenadas estén dentro de los límites del terreno
    terreno_x = max(0, min(ancho - 1, terreno_x))
    terreno_y = max(0, min(alto - 1, terreno_y))
    
    # Iniciar el movimiento del personaje hacia la posición clicada
    iniciar_movimiento_personaje(terreno_x, terreno_y)

# Vincular el evento de clic en la vista isométrica
lienzo.bind("<Button-1>", en_clic_terreno)
...
# Inicializar 130 NPCs en posiciones aleatorias en tierra
for _ in range(1130):
    while True:
