    ...
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            ...
            if 0 <= iso_x < iso_width and 0 <= iso_y < iso_height:
                section.putpixel((iso_x, iso_y), color + (255,))  # Full opacity for land
                depth_section.putpixel((iso_x, iso_y), int(normalized_value * 65535))  # Depth as 16-bit grayscale intensity
    
    # Create the water plane
    water_plane = Image.new("RGBA", (iso_width, iso_height), (0, 0, 255, 128))  # Semi-transparent blue
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            normalized_value = (noise.pnoise3(math.cos(lat) * math.cos(lon) * scale,
                                              math.cos(lat) * math.sin(lon) * scale,
                                              math.sin(lat) * scale,
                                              octaves=16,
                                              persistence=0.5,
                                              lacunarity=2.0) + 1) / 2
            if normalized_value < water_threshold:
                iso_x = int((x - x_start - (y - y_start)) * math.sqrt(3) / 2) + center_x
                iso_y = int((x - x_start + (y - y_start)) / 2 - water_threshold * height_multiplier) + center_y - section_size + int(height_multiplier * 1.3) // 2
                if 0 <= iso_x < iso_width and 0 <= iso_y < iso_height:
                    section.putpixel((iso_x, iso_y), (0, 0, 255, 128))  # Semi-transparent blue for water plane
