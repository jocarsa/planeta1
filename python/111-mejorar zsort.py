...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur, y_pixel_offset, pixel_separation, cloud_offset, shadow_factor, cloud_transparency, cloud_brightness_offset):
    ...
    # Initialize variable to store the character's position
    character_x, character_y = None, None
    character_height = None

    # Calculate the center tile in the terrain section
    center_tile_x = (x_start + x_end) // 2
    center_tile_y = (y_start + y_end) // 2
    
    # First pass: draw the terrain and determine character position
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            ...
            if len(neighbor_iso) == 3:
                # Darken color based on cloud opacity (for terrain)
                cloud_opacity = 0
                if (x, y) in cloud_dict and cloud_dict[(x, y)][1] / 65535.0 > 0.5:
                ...
                # Determine if this is the center tile of the section
                if x == center_tile_x and y == center_tile_y:
                    character_x, character_y = iso_x, iso_y
                    character_height = height

    # Draw the character at the determined position
    if character_x is not None and character_y is not None:
        ...
        # Adjust character position based on terrain height only (no y_pixel_offset here)
        char_x = character_x - scaled_width // 2
        char_y = character_y - scaled_height + int((character_height / 65535.0) * height_multiplier * pixel_separation)

        # Draw the character on the section
        section.paste(scaled_sprite, (char_x, char_y), scaled_sprite)
    
    # Second pass: draw the clouds (on top of everything)
    ...
    # Combine the terrain, character, and cloud layer
...
character_scale = 1.0  # Default scale
