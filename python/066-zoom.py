...
def update_zoom(val):
    global section_size, width, height, x_start, y_start
    section_size = int(int(val) / 100 * min(width, height))
    x_start = (width - section_size) // 2
    y_start = (height - section_size) // 2
    update_canvas()
...
slider_zoom = Scale(root, from_=5, to=10, orient=HORIZONTAL, label="Zoom Level", command=update_zoom)
slider_zoom.set(int(section_size / min(width, height) * 100))
slider_zoom.pack()
