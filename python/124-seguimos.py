...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur, y_pixel_offset, pixel_separation, cloud_offset, shadow_factor, cloud_transparency, cloud_brightness_offset):
    ...
    center_x_iso = iso_width // 2
    center_y_iso = iso_height // 2

    # Get the character's current sprite based on direction
    character_sprite = sprites[character_direction]

    # Calculate the character's position in isometric coordinates
    character_x = x_start + (x_end - x_start) // 2
    character_y = y_start + (y_end - y_start) // 2

    char_iso_x = int(((character_x - x_start - (character_y - y_start)) * math.sqrt(3) / 2) * pixel_separation) + center_x
    char_iso_y = int(((character_x - x_start + (character_y - y_start)) / 2 - (height / 65535.0) * height_multiplier) * pixel_separation) + center_y - int(height_multiplier * pixel_separation * 0.65) + y_pixel_offset

    # Scale the character sprite
    scaled_width = int(sprite_width * character_scale)
    scaled_height = int(sprite_height * character_scale)
    scaled_sprite = character_sprite.resize((scaled_width, scaled_height), Image.Resampling.LANCZOS)


    # Calculate the position to place the scaled character sprite (centered)
    char_x = char_iso_x - scaled_width // 2
    char_y = char_iso_y - scaled_height // 2

    # Draw the character on the section
    section.paste(scaled_sprite, (char_x, char_y), scaled_sprite)
...
class NPC:
    ...
    def move(self):
        ...
        # Exclude the back direction unless the NPC is blocked by water
        possible_directions = possible_moves[self.direction]
        
        # Randomly shuffle but prioritize forward movement, and prevent repetitive same direction turns
        random.shuffle(possible_directions)
        for new_direction in possible_directions:
            if new_direction == self.last_turn and new_direction != possible_directions[0]:
                continue  # Skip if it leads to circular movement
            
            dx, dy = directions[new_direction]
            new_x = (self.x + dx) % width
            new_y = (self.y + dy) % height

            # Check if the new position is land or water
            cur.execute("SELECT height FROM terrain WHERE x = ? AND y = ?", (new_x, new_y))
            result = cur.fetchone()
            if result:
                height_value = result[0] / 65535.0
                if height_value >= water_level:
                    # Move to the new position if it's land
                    self.x = new_x
                    self.y = new_y
                    self.last_turn = None if new_direction == self.direction else new_direction
                    self.direction = new_direction
                    break
            else:
                continue  # If out of bounds, continue to next direction
        
        # Update sprite based on the new direction
        self.sprite = sprites[self.direction]
