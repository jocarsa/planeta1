...
def update_ambient_light():
    global ambient_light, time_of_day
...
# Extract the individual sprites
sprite_width = sprite_sheet.width // 2
sprite_height = sprite_sheet.height // 2

# Dictionary to store the cropped sprites
sprites = {
    "east": sprite_sheet.crop((0, 0, sprite_width, sprite_height)),
    "north": sprite_sheet.crop((sprite_width, 0, sprite_width * 2, sprite_height)),
    "south": sprite_sheet.crop((0, sprite_height, sprite_width, sprite_height * 2)),
    "west": sprite_sheet.crop((sprite_width, sprite_height, sprite_width * 2, sprite_height * 2)),
}

# Global variable for the character's direction
character_direction = "south"  # Default direction

# Global variable for character scale
character_scale = 0.25  # Default scale

# Function to update the canvas with the new section
def update_canvas():
    global canvas, tk_img
    global x_start, y_start, section_size, cur, height_multiplier, y_pixel_offset, pixel_separation, cloud_offset, shadow_factor, cloud_transparency, cloud_brightness_offset
    ...
    # Draw food items visible in the current section
    for food in foods:
        if x_start <= food.x < x_end and y_start <= food.y < y_end:
            # Calculate the isometric position
            iso_x = int(((food.x - x_start - (food.y - y_start)) * math.sqrt(3) / 2) * pixel_separation) + section.width // 2

            # Fetch the height for the current food position from the terrain
            cur.execute("SELECT height FROM terrain WHERE x = ? AND y = ?", (food.x, food.y))
            height_result = cur.fetchone()
            terrain_height = height_result[0] if height_result else 0
            normalized_value = terrain_height / 65535.0

            # Adjust iso_y based on the terrain height
            iso_y = int(((food.x - x_start + (food.y - y_start)) / 2 - normalized_value * height_multiplier) * pixel_separation) + section.height // 2 - int(height_multiplier * pixel_separation * 0.65) + y_pixel_offset
            
            # Draw food item as a small filled circle (or any other icon)
            section_draw = ImageDraw.Draw(section)
            section_draw.ellipse((iso_x - 5, iso_y - 5, iso_x + 5, iso_y + 5), fill="yellow")

    # Draw NPCs visible in the current section
    for npc in npcs:
        if x_start <= npc.x < x_end and y_start <= npc.y < y_end:
            # Calculate the isometric position
            iso_x = int(((npc.x - x_start - (npc.y - y_start)) * math.sqrt(3) / 2) * pixel_separation) + section.width // 2

            # Fetch the height for the current NPC position from the terrain
            cur.execute("SELECT height FROM terrain WHERE x = ? AND y = ?", (npc.x, npc.y))
            height_result = cur.fetchone()
            terrain_height = height_result[0] if height_result else 0
            normalized_value = terrain_height / 65535.0

            # Adjust iso_y based on the terrain height
            iso_y = int(((npc.x - x_start + (npc.y - y_start)) / 2 - normalized_value * height_multiplier) * pixel_separation) + section.height // 2 - int(height_multiplier * pixel_separation * 0.65) + y_pixel_offset

            npc.draw(section, iso_x, iso_y)
...
def pan(dx, dy, step=1):
    global x_start, y_start, width, height, section_size, character_direction
    ...
    # Move the start positions by the step size
    x_start = (x_start + dx * step) % width
    y_start = (y_start + dy * step) % height
    update_canvas()
    draw_sphere()
    update_crosshair()
...
# Function to update the character scale
def update_character_scale(val):
    global character_scale
    character_scale = float(val)
    label_character_scale_value.config(text=f"{character_scale:.2f}")
    update_canvas()
...
def update_crosshair():
    ...
    draw.line([(eq_x - 25, eq_y), (eq_x + 25, eq_y)], fill="red")
    draw.line([(eq_x, eq_y - 25), (eq_x, eq_y + 25)], fill="red")
...
# Preprocess equirectangular map for pathfinding
def preprocess_equirectangular_map():
    global equirect_map, equirect_pathfinding_grid
    eq_width, eq_height = equirect_map.size
    equirect_pathfinding_grid = np.zeros((eq_width, eq_height))
    
    for x in range(eq_width):
        for y in range(eq_height):
            terrain_x = int((x / eq_width) * width)
            terrain_y = int((y / eq_height) * height)
            cur.execute("SELECT height FROM terrain WHERE x = ? AND y = ?", (terrain_x, terrain_y))
            result = cur.fetchone()
            if result and result[0] / 65535.0 >= water_level:
                equirect_pathfinding_grid[x, y] = 1  # Passable
            else:
                equirect_pathfinding_grid[x, y] = 0  # Impassable (water)

# Initialize height multiplier, y pixel offset, cloud offset, shadow factor, and cloud brightness offset
height_multiplier = 150
y_pixel_offset = 1000
cloud_offset = 16  # Default cloud offset
shadow_factor = 0.3  # Default shadow darkening factor
cloud_transparency = 1.0  # Default cloud transparency
cloud_brightness_offset = 154  # Default cloud brightness offset
...
# Create a slider to adjust the character scale
label_character_scale = ttk.Label(toolbar, text="Escala del personaje")
label_character_scale.grid(row=0, column=16, padx=5)
slider_character_scale = ttk.Scale(toolbar, from_=0.1, to=2.0, orient=HORIZONTAL, command=update_character_scale)
slider_character_scale.set(character_scale)
slider_character_scale.grid(row=1, column=16, padx=5)
label_character_scale_value = ttk.Label(toolbar, text=f"{character_scale:.2f}")
label_character_scale_value.grid(row=1, column=17, padx=5)
...
# Bind arrow keys for panning, with Shift for faster movement
def key_press(event):
    step = 5 if event.state & 0x0001 else 1  # Shift modifier check, using bitmask for shift key
    if event.keysym == 'Up':
        pan(0, -1, step)
...
# Function to spawn a food item at a random location (never on water)
class Food:
...
def spawn_food():
    while True:
        ...
        cur.execute("SELECT height FROM terrain WHERE x = ? AND y = ?", (x, y))
        result = cur.fetchone()
        if result and result[0] / 65535.0 >= water_level:  # Ensure the food is not on water
            return Food(x, y)

# List to store food items
foods = [spawn_food() for _ in range(5)]  # Initialize with 5 food items

# A* pathfinding algorithm
def heuristic(a, b):
    return abs(a[0] - b[0]) + abs(a[1] - b[1])

def a_star_search_equirect(start, goal):
    frontier = []
    ...
    while frontier:
        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if 0 <= next_node[0] < eq_width and 0 <= next_node[1] < eq_height:
                if equirect_pathfinding_grid[next_node[0], next_node[1]] == 1:  # Check if passable
    ...
    # Convert path back to original terrain coordinates
    converted_path = [(int(px * width / eq_width), int(py * height / eq_height)) for px, py in path]
    return converted_path if converted_path[0] == start else []
...
class NPC:
    def __init__(self, x, y, direction="south"):
        ...
        self.sprite = sprites[self.direction]
        self.last_turn = None  # To track the last turn direction
        self.path = []  # Store the path to the target food

    def move(self):
        if not self.path:
            # Find nearest food
            nearest_food = min(foods, key=lambda food: heuristic((self.x, self.y), (food.x, food.y)))
            start_eq = (int(self.x * equirect_map.size[0] / width), int(self.y * equirect_map.size[1] / height))
            goal_eq = (int(nearest_food.x * equirect_map.size[0] / width), int(nearest_food.y * equirect_map.size[1] / height))
            self.path = a_star_search_equirect(start_eq, goal_eq)
        ...
        if self.path:
            ...
            # Check if NPC has reached the food
            if (self.x, self.y) == (nearest_food.x, nearest_food.y):
                foods.remove(nearest_food)
                foods.append(spawn_food())  # Respawn a new food item
                self.path = []  # Reset path

    def draw(self, canvas, iso_x, iso_y):
        # Scale the sprite based on the global character_scale
        scaled_sprite = self.sprite.resize((int(sprite_width * character_scale), int(sprite_height * character_scale)), Image.Resampling.LANCZOS)
        canvas.paste(scaled_sprite, (iso_x - scaled_sprite.width // 2, iso_y - scaled_sprite.height // 2), scaled_sprite)
    
    def draw_on_map(self, draw, eq_width, eq_height):
        # Draw crosshair on the equirectangular map
        eq_x = int((self.x / width) * eq_width)
        eq_y = int((self.y / height) * eq_height)
        draw.line([(eq_x - 25, eq_y), (eq_x + 25, eq_y)], fill="red")
        draw.line([(eq_x, eq_y - 25), (eq_x, eq_y + 25)], fill="red")
...
# Initialize 30 NPCs at random positions on land
for _ in range(30):
    while True:
        x = random.randint(0, width - 1)
        y = random.randint(0, height - 1)
...
def update_npcs():
    global canvas, tk_img, eq_map_label, equirect_img
    
    for npc in npcs:
        npc.move()
    
    # Redraw the isometric view with NPCs
    update_canvas()
    
    # Redraw the equirectangular map with NPCs' positions
    eq_width, eq_height = equirect_map.size
    draw = ImageDraw.Draw(equirect_map)
    for npc in npcs:
        npc.draw_on_map(draw, eq_width, eq_height)
    
    # Convert the updated map to ImageTk format and update the label
    equirect_img = ImageTk.PhotoImage(equirect_map)
    eq_map_label.config(image=equirect_img)
    eq_map_label.image = equirect_img

    # Schedule the next update
    root.after(1000, update_npcs)
...
root.after(100, preprocess_equirectangular_map)
root.after(100, update_time)
root.after(1000, update_npcs)  # Start NPC updates
root.mainloop()
