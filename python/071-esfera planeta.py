...
def draw_sphere():
    ...
    for i in range(num_parallels + 1):
        lat = (i / num_parallels) * math.pi - math.pi / 2
        for j in range(num_meridians):
            lon = (j / num_meridians) * 2 * math.pi
            x = radius * math.cos(lat) * math.cos(lon)
            y = radius * math.cos(lat) * math.sin(lon)
            z = radius * math.sin(lat)
            
            # Map the 3D coordinates to 2D using simple projection
            screen_x = center_x + x / (1 + z / (2 * radius))
            screen_y = center_y - y / (1 + z / (2 * radius))
            
            # Determine the color based on the database entry
            ...
            if result:
                ...
                sphere_canvas.create_oval(screen_x - 2, screen_y - 2, screen_x + 2, screen_y + 2, fill=color, outline=color)
