...
def update_canvas():
    ...
    for npc in npcs:
        if x_start <= npc.x < x_end and y_start <= npc.y < y_end:
            ...
            iso_y = int(((npc.x - x_start + (npc.y - y_start)) / 2) * pixel_separation) + section.height // 2 - int(height_multiplier * pixel_separation * 0.65) + y_pixel_offset
    ...
    def move(self):
        ...
        if result:
            ...
            if height_value >= water_level:
                ...
                # If it's water, choose a new direction
...
# Define a spawn range around the player's starting position
spawn_range = section_size // 4  # Example: NPCs can spawn within a quarter of the section size

# Initialize 30 NPCs at random positions near the player's starting position
for _ in range(30):
    while True:
        # Generate random positions within the spawn range
        x = (x_start + random.randint(-spawn_range, spawn_range)) % width
        y = (y_start + random.randint(-spawn_range, spawn_range)) % height
