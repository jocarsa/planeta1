...
def generate_section(args):
    ...
    for x in range(x_start, x_end):
        for y in range(height):
            ...
            nz = math.sin(lat)
            perlin_value = noise.pnoise3(nx * scale, ny * scale, nz * scale, octaves=18, persistence=0.5, lacunarity=2.0)
            ...
            if normalized_value >= 0.4:
                ...
                elif abs(lat) > math.pi / 4:  # Snow cover with noise
                    snow_factor = ((abs(lat) - math.pi / 4) / (math.pi / 20)) * noise_factor
