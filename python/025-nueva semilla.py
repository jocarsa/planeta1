...
def generate_section(args):
    x_start, x_end, width, height, scale, base_seed, water_level = args
    ...
    for x in range(x_start, x_end):
        for y in range(height):
            ...
            # Generate a new seed for each coordinate
            seed = base_seed + x + y * width
            
            # Generate Perlin noise value for the spherical coordinates
            nx = math.cos(lat) * math.cos(lon)
            ny = math.cos(lat) * math.sin(lon)
            nz = math.sin(lat)
            perlin_value = noise.pnoise3(nx * scale, ny * scale, nz * scale, octaves=16, persistence=0.5, lacunarity=2.0, base=seed)
...
def main():
    ...
    tasks = [(i * section_width, (i + 1) * section_width if i < num_sections - 1 else width, width, height, scale, base_seed, water_level) for i in range(num_sections)]
    ...
    print(f"Images saved as {diffuse_filename} and {depth_filename}")
