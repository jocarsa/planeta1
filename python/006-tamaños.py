...
def generate_perlin_noise_image(hue, width, height, scale=10):
...
for _ in range(10):
    ...
    scale_factor = random.randint(1, 40)  # Random scale factor between 1 and 10
    width = base_width * scale_factor
    height = base_height * scale_factor
    
    # Generate the Perlin noise image with the selected hue
    image = generate_perlin_noise_image(hue, width, height)

    # Scale the image to 1920x1080 without antialiasing
    scaled_image = image.resize((8192, 4320), Image.Resampling.NEAREST)
