...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur):
    ...
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            # Check if the pixel data exists in the database
            cur.execute("SELECT color, height FROM terrain WHERE x=? AND y=?", (x, y))
            row = cur.fetchone()
            if row:
                color = tuple(map(int, row[0].split(',')))
                normalized_value = row[1] / 65535.0
            else:
                ...
                # Save the pixel data to the database
...
section_size = int(math.sqrt(0.0005 * width * height))
