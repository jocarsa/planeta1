...
escala_personaje = 0.25  # Escala por defecto del personaje
...
def desplazar(dx, dy, paso=1):
    ...
    actualizar_crosshair()  # Actualizar la cruceta
...
# Crear el lienzo para el mapa equirectangular (solo se dibuja una vez)
canvas_mapa_eq = ttk.Canvas(marco_izquierdo, width=480, height=240)
canvas_mapa_eq.grid(row=1, column=0, padx=10, pady=10)

# Si el fondo del marco debe coincidir con el fondo del canvas
canvas_crosshair = ttk.Canvas(marco_izquierdo, width=480, height=240)
canvas_crosshair.place(x=10, y=260, anchor="nw")
canvas_crosshair.configure(highlightthickness=0, bg=raiz.cget("background"))
...
def dibujar_mapa_equirectangular():
    ...
    canvas_mapa_eq.create_image(0, 0, anchor=NW, image=img_eq)
    canvas_mapa_eq.image = img_eq

# Función para actualizar la cruceta en el canvas de crosshairs
def actualizar_crosshair():
    global npcs, canvas_crosshair, ancho, alto
    
    # Limpiar el canvas de crosshair
    canvas_crosshair.delete("all")
    
    ancho_eq, alto_eq = 480, 240  # Dimensiones del canvas de crosshairs
    
    for npc in npcs:
        # Calcular la posición en el canvas
        eq_x = int((npc.x / ancho) * ancho_eq)
        eq_y = int((npc.y / alto) * alto_eq)
        
        # Dibujar la cruceta
        canvas_crosshair.create_line(eq_x - 5, eq_y, eq_x + 5, eq_y, fill="red")
        canvas_crosshair.create_line(eq_x, eq_y - 5, eq_x, eq_y + 5, fill="red")

# Función para manejar clics en el mapa equirectangular
def en_clic_mapa_eq(evento):
    global x_inicio, y_inicio, ancho, alto, tamano_seccion
    ...
    actualizar_lienzo()
    dibujar_esfera()
    actualizar_crosshair()

# Vincular el evento de clic al canvas del mapa equirectangular
canvas_mapa_eq.bind("<Button-1>", en_clic_mapa_eq)
...
# Inicializar NPCs en posiciones aleatorias en tierra
for _ in range(130):
    while True:
...
def actualizar_npcs():
    ...
    # Actualizar solo el canvas de los crosshairs
    actualizar_crosshair()
