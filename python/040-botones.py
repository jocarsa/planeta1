...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier):
    ...
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            ...
            iso_y = int((x - x_start + (y - y_start)) / 2 - normalized_value * height_multiplier) + center_y - section_size+int(height_multiplier*1.7) // 2
...
btn_up_left = Button(root, text="↖", command=lambda: pan(-int(0.001 * width), -int(0.001 * height)))
btn_up_right = Button(root, text="↗", command=lambda: pan(int(0.001 * width), -int(0.001 * height)))
btn_down_left = Button(root, text="↙", command=lambda: pan(-int(0.001 * width), int(0.001 * height)))
btn_down_right = Button(root, text="↘", command=lambda: pan(int(0.001 * width), int(0.001 * height)))

# Place buttons at the corners of the canvas
btn_up_left.place(relx=0, rely=0, anchor="nw")
btn_up_right.place(relx=1, rely=0, anchor="ne")
btn_down_left.place(relx=0, rely=1, anchor="sw")
btn_down_right.place(relx=1, rely=1, anchor="se")
