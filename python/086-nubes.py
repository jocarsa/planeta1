...
def generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur, y_pixel_offset, pixel_separation, cloud_data):
    random.seed(seed)
    ...
    # Third pass: draw the cloud layer
    cloud_layer = Image.new("RGBA", (iso_width, iso_height), (0, 0, 0, 0))
    draw_cloud = ImageDraw.Draw(cloud_layer)
    
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            cloud_value = cloud_data[x % width, y % height]
            if cloud_value > 0.5:  # Adjust the threshold for cloud density
                cloud_color = int(cloud_value * 255)
                iso_x = int(((x - x_start - (y - y_start)) * math.sqrt(3) / 2) * pixel_separation) + center_x
                iso_y = int(((x - x_start + (y - y_start)) / 2) * pixel_separation) + center_y + y_pixel_offset  # Clouds are flat, no height adjustment
                
                # Draw the cloud pixel
                draw_cloud.point((iso_x, iso_y), fill=(255, 255, 255, cloud_color))
    
    # Combine the terrain, water surface, and cloud layer
    section = Image.alpha_composite(section.convert("RGBA"), cloud_layer)
    
    return section.convert("RGB")

# Function to generate a cloud layer
def generate_cloud_layer(width, height, scale, seed):
    random.seed(seed)
    cloud_layer = np.zeros((width, height))
    for x in range(width):
        for y in range(height):
            nx, ny = x / width - 0.5, y / height - 0.5
            cloud_layer[x, y] = noise.pnoise2(nx * scale, ny * scale, octaves=6, persistence=0.6, lacunarity=2.0, repeatx=width, repeaty=height, base=seed)
    cloud_layer = (cloud_layer + 1) / 2  # Normalize to [0, 1]
    return cloud_layer
...
# Generate the cloud data once at the start
cloud_data = generate_cloud_layer(width, height, scale, seed + 1)  # Use a different seed for clouds

# Function to update the canvas with the new section
def update_canvas():
    global x_start, y_start, section_size, canvas, tk_img, cur, height_multiplier, y_pixel_offset, pixel_separation, cloud_data
    x_end = x_start + section_size
    y_end = y_start + section_size
    
    # Generate the section in isometric perspective
    section = generate_section_isometric(x_start, x_end, y_start, y_end, width, height, scale, seed, water_level, height_multiplier, cur, y_pixel_offset, pixel_separation, cloud_data)
